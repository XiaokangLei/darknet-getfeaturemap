...
network *load_network_binary(char *cfg, char *weights, int clear)
{
    network *net = parse_network_cfg(cfg);
    if(weights && weights[0] != 0){
        load_weights_binary(net, weights);
    }
    if(clear) (*net->seen) = 0;
    return net;
}
...
image float_to_cow_image_float(int w, int h, int c, float *data, int ai)
{
	char tp[1000];
	...
	for (int k = 0; k < c; k++) {
		...
		mkdir(tp,0777);
		if (ai < 10)
			sprintf(tp, "feature_txt/%d", ai);
		else
			sprintf(tp, "feature_txt/%d", ai);
		mkdir(tp,0777);
		if (ai < 10)
			sprintf(tp, "feature_txt/%d/%d_%d.txt", ai, ai, k);
		else
			sprintf(tp, "feature_txt/%d/%d_%d.txt", ai, ai, k);
...
image get_network_cow_image_layer_float(network *net, int i)
{
	layer l = net->layers[i];
	printf("test1:  w:%d,h:%d,c:%d\n", l.out_w, l.out_h, l.out_c);
	if (l.out_w && l.out_h && l.out_c) {
		return float_to_cow_image_float(l.out_w, l.out_h, l.out_c, l.output, i);
	}
	image def = { 0 };
	return def;
}
...
void forward_network(network *netp)
{
    ...
    for(i = 0; i < net.n; ++i){
        ...
        l.forward(l, net);
        net.input = l.output;
        image tmp = get_network_cow_image_layer_float(&net, i);
        if(l.truth) {
            net.truth = l.output;
        }
    }
