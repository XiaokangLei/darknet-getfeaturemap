...
// [仿真]指数动态固定的
float SimTrim2FixedPoint_mu(int num, int bit_width, float *data)
{
	int i, index;
	float max_weight, fl;
	int maxIndex = 0;
	...
	printf("weights:%f,%f,", data[maxIndex], data[minIndex]);
	if (fabs(data[maxIndex]) < fabs(data[minIndex]))
		max_weight = fabs(data[minIndex]);
	else
		max_weight = fabs(data[maxIndex]);
	
    fl = pow(2,(bit_width-1))/max_weight;
    printf("fl:%f\n", fl);
	float data_temp;
	float max_data = (pow(2,(bit_width-1))-1)/fl;
	float min_data = -pow(2,(bit_width-1))/fl;
    int temp = 0;
	for (index = 0; index < num; index++) {
		data_temp = max(min(data[index], max_data), min_data);
		data_temp *= fl;
        temp = (int)round(data_temp);
		data[index] = temp/fl;
	}
	return fl;
...
// [仿真]指数动态固定的
int SimTrim2FixedPoint_ex(int num, int bit_width, float *data)
{
	int i, fl, index;
	float max_weight;
	int maxIndex = 0;
	...
	for (index = 0; index < num; index++) {
		...
		data_temp *= pow(2, fl);
        temp = (int)round(data_temp);
		data[index] = temp*(pow(2, -fl));
	}
	return fl;

}
// 指数动态固定的
int Trim2FixedPoint_ex(int num, int bit_width, float *data, signed char *data_out)
{
	int i, fl, index;
	float max_weight;
	int maxIndex = 0;
...
void save_convolutional_weights_nobn(layer l, FILE *fp)
{
    ...
    fwrite(l.biases, sizeof(float), l.n, fp);
    //int fl_w = Trim2FixedPoint_ex(num, 8, l.weights, l.weights_ch);
    //printf("fl_w:%d\n",fl_w);
    //fwrite(l.weights_ch, sizeof(signed char), num, fp);
    fwrite(l.weights, sizeof(float), num, fp);
}
...
void save_weights_upto_nobn(network *net, char *filename, int cutoff)
{
#ifdef GPU
    if(net->gpu_index >= 0){
        cuda_set_device(net->gpu_index);
    ...
    for(i = 0; i < net->n && i < cutoff; ++i){
        ...
        if (l.dontsave) continue;
        if(l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL){
            save_convolutional_weights_nobn(l, fp);
        } if(l.type == CONNECTED){
            save_connected_weights(l, fp);
        } if(l.type == BATCHNORM){
            save_batchnorm_weights(l, fp);
...
void save_weights_upto_binary(network *net, char *filename, int cutoff)
{
#ifdef GPU
    if(net->gpu_index >= 0){
        cuda_set_device(net->gpu_index);
...
void save_weights_nobn(network *net, char *filename)
{
    save_weights_upto_nobn(net, filename, net->n);
}
void save_weights_binary(network *net, char *filename)
{
    save_weights_upto_binary(net, filename, net->n);
}
...
void load_convolutional_weights(layer l, FILE *fp)
{
    ...
    //binarize_weights(l.weights, l.n, l.c*l.size*l.size, l.weights);
#ifdef GPU
    if(gpu_index >= 0){
        push_convolutional_layer(l);
    }
...
void load_weights_upto_binary(network *net, char *filename, int start, int cutoff)
{
#ifdef GPU
    if(net->gpu_index >= 0){
        cuda_set_device(net->gpu_index);
...
void load_weights_upto_nobn(network *net, char *filename, int start, int cutoff)
{
#ifdef GPU
    if(net->gpu_index >= 0){
        cuda_set_device(net->gpu_index);
    ...
    for(i = start; i < net->n && i < cutoff; ++i){
        ...
        if (l.dontload) continue;
        if(l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL){
            load_convolutional_weights_nobn(l, fp);
        }
        if(l.type == CONNECTED){
            load_connected_weights(l, fp, transpose);
        }
...
void load_weights_binary(network *net, char *filename)
{
    load_weights_upto_binary(net, filename, 0, net->n);
}

void load_weights_nobn(network *net, char *filename)
{
    load_weights_upto_nobn(net, filename, 0, net->n);
}
