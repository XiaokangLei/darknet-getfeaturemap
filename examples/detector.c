...
void test_detector_comBN(char *datacfg, char *cfgfile, char *weightfile, char *filename,char *weightname ,float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    network *net_b = load_network_nobn(cfgfile, weightname, 0);
    set_batch_network(net_b, 1);
    srand(2222222);
    double time;
    char buff[256];
    char *input = buff;
    ...
    while(1){
        ...
        image im = load_image_color(input,0,0);
        image sized = letterbox_image(im, net_b->w, net_b->h);
        //image sized = resize_image(im, net->w, net->h);
        //image sized2 = resize_max(im, net->w);
        //image sized = crop_image(sized2, -((net->w - sized2.w)/2), -((net->h - sized2.h)/2), net->w, net->h);
        //resize_network(net, sized.w, sized.h);
        layer l = net_b->layers[net_b->n-1];


        float *X = sized.data;
        time=what_time_is_it_now();
        network_predict(net_b, X);
        printf("%s: Predicted in %f seconds.\n", input, what_time_is_it_now()-time);
        int nboxes = 0;
        detection *dets = get_network_boxes(net_b, im.w, im.h, thresh, hier_thresh, 0, 1, &nboxes);
        //printf("%d\n", nboxes);
        //if (nms) do_nms_obj(boxes, probs, l.w*l.h*l.n, l.classes, nms);
        if (nms) do_nms_sort(dets, nboxes, l.classes, nms);
        draw_detections(im, dets, nboxes, thresh, names, alphabet, l.classes);
...
void test_detector_saveBinary(char *datacfg, char *cfgfile, char *weightfile, char *filename,char *weightname ,float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
    ...
    save_weights_binary(net, weightname);

    network *net_b = load_network_binary(cfgfile, weightname, 0);
    set_batch_network(net_b, 1);
    srand(2222222);
    double time;
    char buff[256];
...
void run_detector(int argc, char **argv)
{
    ...
    char *weightname = (argc > 7) ? argv[7]: 0;
    ...
    else if(0==strcmp(argv[2], "saveBinary")) test_detector_saveBinary(datacfg, cfg, weights, filename, weightname,thresh, hier_thresh, outfile, fullscreen);
